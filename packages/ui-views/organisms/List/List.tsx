      ...
      <section {...currentTheme(1, 'list_spaced')}>
        ...
        {events.map((item, i) => (
          <div {...currentTheme(i+i+i+i+i+i+4, 'list_item')}>
            ...
            <h2 {...currentTheme(i+i+i+i+i+i+7, 'item_title')}><a>{item.provider.name}</a> {item.action} {item.affectedQuantity.numericValue} {item.affectedQuantity.unit ? item.affectedQuantity.unit.name : ''} • <b>{item.start}</b> </h2>
            </div>
